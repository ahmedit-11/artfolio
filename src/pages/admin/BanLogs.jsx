...
const BanLogs = () => {
  ...
  const banlog = Array.isArray(users) && users.length>0 &&users.filter(user=>user)
console.log(banlog)
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <StatCard
            title="Banned Users"
            value={users.filter(log => log.banned ==1).length}
            icon={Activity}
            color="orange"
          />
          <StatCard
      ...
      <div className="overflow-x-auto -mx-4 sm:mx-0">
        <div className="inline-block min-w-full align-middle">
          <table className="min-w-full">
            ...
            <tbody>
              {Array.isArray(users)&&users.length>0&&users.map((log,i) => (
                  ...
                  <td className="py-3 sm:py-4 px-2 sm:px-4">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <Avatar className="size-6 sm:size-8 flex-shrink-0">
                         <AvatarImage src={mockBanLogs[i%4].avatar} alt={log.name} />
                       </Avatar>
                      <div className="min-w-0 flex-1">
                        <div className="font-medium text-foreground text-sm sm:text-base truncate">{log.name}</div>
                        <div className="text-xs text-muted-foreground truncate">{log.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 sm:py-4 px-2 sm:px-4 text-xs sm:text-sm text-muted-foreground hidden lg:table-cell">
                <span className={`p-1 rounded-[10px] text-white ${log.banned ? 'bg-green-400':'bg-red-300'}`}>{log.banned?"banned":"unbanned"}</span>
                  </td>
                  <td className="py-3 sm:py-4 px-2 sm:px-4">
                    <button
